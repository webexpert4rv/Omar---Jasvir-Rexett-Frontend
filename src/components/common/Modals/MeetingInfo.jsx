...
const MeetingInfo = ({ show, handleClose, details }) => {
  ...
  const [rexettInfo, setRexettInfo] = useState()
  ...
  const { rexettMeetingData } = useSelector(state => state.adminData)
  console.log(rexettMeetingData, "rexettMeetingData")
  const { interview: {
    id, // application id
    title,
    developer_name,
  ...
  const checkInterviewStatus = async (eventId, job_external_id) => {
    ...
    if (meeting_platform === "microsoft_team") {
      ...
      dispatch(getDetailsByRexettMeeting(job_external_id))
    }
    setRexettInfo(rexettMeetingData);
    setShowDetailsSection(true);
  ...
  console.log(details, "details")
  console.log(rexettInfo, "rexettInfo")
          ...
          <div className="d-flex justify-content-between align-items-center">
            ...
            <div>
              <Button
                ...
                onClick={() => checkInterviewStatus(details?.interview?.event_id, details?.interview?.job_external_id)}
              >
              ...
              <Row>
                <Col lg={8}>
                  <div className="call-duration">
                    Call Duration : {info?.callDuration || rexettInfo?.eventDuration}
                  </div>
                  <div className="attendees">
                    <p>Attendees : </p>
                    {rexettInfo?.attendees?.length > 0 ?
                      rexettInfo?.attendees?.map((data) => (
